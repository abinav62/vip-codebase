...
There is no stoploss or trailing stoploss set for this strategy. The strategy runs on
...
def simulate_intraday_straddle(ce, pe, pl_data, underlying):
    """
    ...
    try:
        ...
        for i in range(11, 16):
            ce_close = ce['c'][(ce['t'].dt.time >= time(i)) & (ce['t'].dt.time < time(i + 1))].iloc[0]
            pe_close = pe['c'][(pe['t'].dt.time >= time(i)) & (pe['t'].dt.time < time(i + 1))].iloc[0]
            pl_data = pd.concat([pl_data, pd.DataFrame([{
                'date': ce['t'][ce['t'].dt.time == time(i)].iloc[0],
                'straddle_entry': ce_entry + pe_entry,
                'straddle_exit': pe_close + pe_close,
                'pl_points': ce_entry - ce_close + pe_entry - pe_close,
                'pl_dollars': (ce_entry - ce_close + pe_entry - pe_close) * 100,
            }])], ignore_index=True)

        # Append the data to the pl_data dataframe
        return pl_data
    except Exception as e:
        print(e)
        # if len(ce[ce['t'].dt.minute == 0]) < 5:
        #     print(ce[ce['t'].dt.minute == 1])
        # if len(pe[pe['t'].dt.minute == 0]) < 5:
        #     print(pe[pe['t'].dt.minute == 1])
...
def main():
    og_df = clean_data(get_s3_file('vip-mlfs-index-data', 'SPX.csv'))
    df = clean_data(get_s3_file('vip-mlfs-index-data', 'SPX_updated.csv'))
    df = pd.concat([og_df, df], ignore_index=True)
    dates = df[(df['datetime'].dt.time == time(10)) & (df['datetime'].dt.date >= date(2023, 10, 1))]
    pl = 0
    ...
    for i, row in dates.iterrows():
        ...
        try:
            ...
            pl_data = simulate_intraday_straddle(ce, pe, pl_data, df[df['datetime'].dt.date == row['datetime'].date()])
        except Exception as e:
            print(e)
            continue

    # Export pl_data to csv
    pl_data.to_csv('../data/vanilla_straddle_updated.csv', index=False, header=True)
