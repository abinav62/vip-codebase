"""
This file contains data visualizations of SPX data and its indicators.
"""
...
def main():
    ...
    df = get_s3_file('vip-mlfs-stock-data', f'{symbol}_updated.csv')
    df = clean_data(df)
    original_df = pd.read_csv(f'../../data/stocks/{symbol}_cleaned.csv')
    df['date'] = pd.to_datetime(df['date'])
    original_df['date'] = pd.to_datetime(original_df['date'])
    new_df = pd.concat([original_df, df], ignore_index=True)
    new_df['date'] = pd.to_datetime(new_df['date'])
    cleaned_df = get_indicators(convert_to_hourly(new_df))
    cleaned_df.to_csv(f'../../data/stocks/{symbol}_updated_cleaned.csv', index=False, header=True)

    # plot_data(df.drop('volume', axis=1))
    # plot_selected(df, ['date', 'supertrend'], '2021-10-01', '2021-10-31')
